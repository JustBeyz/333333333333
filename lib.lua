...
local CustomImageManagerAssets = {
    ...
    WatermarkIcon = {
        FileName = "WatermarkIcon.png",
        URL = "https://raw.githubusercontent.com/JustBeyz/333333333333/refs/heads/main/img3d3.png",

        Id = nil,
    }
}

local function ErrorFriendlyPCall(Callback, ...)
    local Results = table.pack(xpcall(Callback, function(Error)
        return debug.traceback(tostring(Error), 2)
    end, ...))

    if not Results[1] then
        warn("[Forma Library] protected call failed:\n" .. tostring(Results[2]))
    end
...
do
    local function RecursiveCreatePath(Path: string, IsFile: boolean?)
        ...
        for _, Segment in ipairs(Segments) do
            local FolderPath = TraversedPath .. Segment
            local Exists = false
            if isfolder then
                local Success, IsExisting = ErrorFriendlyPCall(isfolder, FolderPath)
                Exists = Success and IsExisting == true
            end

            if not Exists then
                ErrorFriendlyPCall(makefolder, FolderPath)
            end
    ...
    local function UpdateAssetPath(AssetData)
        AssetData.Path = AssetStorage.Images .. "/" .. AssetData.FileName
    end

    for _, AssetData in next, CustomImageManagerAssets do
        UpdateAssetPath(AssetData)
    end
    ...
    function CustomImageManager.AddAsset(AssetName: string, RobloxAssetId: number?, URL: string?, ForceRedownload: boolean?)
        ...
        assert(RobloxAssetId == nil or typeof(RobloxAssetId) == "number", "RobloxAssetId must be a number or nil")
        ...
        UpdateAssetPath(CustomImageManagerAssets[AssetName])
    ...
    function CustomImageManager.GetAsset(AssetName: string)
        ...
        local HasFile = false
        if isfile then
            local Success, Exists = ErrorFriendlyPCall(isfile, AssetData.Path)
            HasFile = Success and Exists == true
        end

        if AssetData.URL and not HasFile then
            local DownloadSuccess = CustomImageManager.DownloadAsset(AssetName)
            if isfile then
                local Success, Exists = ErrorFriendlyPCall(isfile, AssetData.Path)
                HasFile = (Success and Exists == true) or DownloadSuccess == true
            else
                HasFile = DownloadSuccess == true
            end
        end

        local AssetID = if AssetData.RobloxId then string.format("rbxassetid://%s", AssetData.RobloxId) else nil

        if getcustomasset and (HasFile or not isfile) then
            local Success, NewID = ErrorFriendlyPCall(getcustomasset, AssetData.Path)
        ...
        if not AssetID and AssetData.URL then
            warn("[Forma Library] asset " .. AssetName .. " has no usable asset id. Check getcustomasset and " .. AssetData.Path)
        end
    ...
    function CustomImageManager.DownloadAsset(AssetName: string, ForceRedownload: boolean?)
        ...
        if not AssetData then
            warn("[Forma Library] asset download failed: invalid asset " .. tostring(AssetName))
            return false, "invalid asset"
        end

        if not AssetData.URL then
            return true, nil
        end

        if not writefile then
            warn("[Forma Library] asset " .. AssetName .. " could not load: writefile is unavailable")
            return false, "writefile is unavailable"
        end

        RecursiveCreatePath(AssetData.Path, true)

        if ForceRedownload ~= true and isfile then
            local Success, Exists = ErrorFriendlyPCall(isfile, AssetData.Path)
            if Success and Exists == true then
                return true, nil
            end
        end
        ...
        if not success then
            warn("[Forma Library] asset " .. AssetName .. " failed at " .. AssetData.Path .. ": " .. tostring(errorMessage))
        end

        return success, errorMessage
    end

    function CustomImageManager.SetAssetURL(AssetName: string, URL: string)
        local AssetData = CustomImageManagerAssets[AssetName]
        if not AssetData or typeof(URL) ~= "string" or URL == "" then
            return false
        end

        AssetData.URL = URL
        AssetData.Id = nil
        return CustomImageManager.DownloadAsset(AssetName, true)
    end

    function CustomImageManager.RefreshAssets()
        for AssetName, AssetData in next, CustomImageManagerAssets do
            UpdateAssetPath(AssetData)
            AssetData.Id = nil
...
function Library:SafePCall(Func, ...)
    if not (Func and typeof(Func) == "function") then
        local Error = "expected a function"
        warn("[Forma Library] protected call failed: " .. Error)
        return false, Error
    end

    local Result = table.pack(ErrorFriendlyPCall(Func, ...))
    if not Result[1] then
        if Library.NotifyOnError and typeof(Library.Notify) == "function" then
            Library:Notify({
                Title = "Library error",
                Description = tostring(Result[2]),
                Time = 6,
            })
        end
...
function Library:SafeCallback(Func, ...)
    ...
    local Result = table.pack(Library:SafePCall(Func, ...))
    if not Result[1] then
        return nil
    end
...
function Library:Create(Class, Properties)
    ...
    for Property, Value in next, Properties do
        if Property == "Font" and Library.FontFace then
            Property = "FontFace"
            Value = Library.FontFace
        elseif Property == "Size" or Property == "Position" then
            Value = ApplyDPIScale(Value)
        elseif Property == "TextSize" then
            Value = ApplyTextScale(Library.FontSize)
        end
    ...
    if _Instance:IsA("TextLabel") or _Instance:IsA("TextBox") or _Instance:IsA("TextButton") then
        Library:ApplyFontOffset(_Instance)
    end
...
function Library:ApplyFontOffset(TextInstance)
    if not TextInstance or not (TextInstance:IsA("TextLabel") or TextInstance:IsA("TextBox") or TextInstance:IsA("TextButton")) then
        return
    end

    local Padding = TextInstance:FindFirstChild("FormaFontPadding")
    if not Padding then
        Padding = Instance.new("UIPadding")
        Padding.Name = "FormaFontPadding"
        Padding.Parent = TextInstance
    end

    pcall(function()
        local Offset = math.max(1, math.floor(DPIScale + 0.5))
        Padding.PaddingTop = UDim.new(0, Offset)
        Padding.PaddingBottom = UDim.new(0, -Offset)
    end)
end

function Library:UpdateFont()
    for _, Descendant in next, ScreenGui:GetDescendants() do
        if Descendant:IsA("TextLabel") or Descendant:IsA("TextBox") or Descendant:IsA("TextButton") then
            Library:ApplyFont(Descendant)
            Library:ApplyFontOffset(Descendant)
        end
    end
end
...
function Library:GetAssetStorage()
    return {
        Images = AssetStorage.Images;
        Fonts = AssetStorage.Fonts;
    }
end

function Library:SetAssetStorage(Storage)
    local Images = AssetStorage.Images
    local Fonts = AssetStorage.Fonts

    if typeof(Storage) == "string" then
        local Root = Storage:gsub("\\", "/"):gsub("/+$", "")
        if Root == "" then
        ...
        Images = Root .. "/images"
        Fonts = Root .. "/fonts"
    elseif typeof(Storage) == "table" then
        local Root = typeof(Storage.Root) == "string" and Storage.Root:gsub("\\", "/"):gsub("/+$", "") or nil
        Images = Storage.Images or (Root and Root .. "/images") or Images
        Fonts = Storage.Fonts or (Root and Root .. "/fonts") or Fonts

        if typeof(Images) ~= "string" or typeof(Fonts) ~= "string" or Images == "" or Fonts == "" then
            return false, "Images and Fonts must be non-empty paths"
        end

        Images = Images:gsub("\\", "/"):gsub("/+$", "")
        Fonts = Fonts:gsub("\\", "/"):gsub("/+$", "")
    else
        return false, "expected a path string or { Images, Fonts } table"
    end

    AssetStorage.Images = Images
    ...
    if Library.WatermarkIconImage and Library.SetWatermarkIcon then
        Library:SetWatermarkIcon(Library.WatermarkIconSource, Library.WatermarkIconEnabled)
    end

    return true, Library:GetAssetStorage()
end

local ActiveTweens = {}
local PopupVisualState = {}
local PopupPositions = {}
local PopupAnimationTokens = {}
local PopupOpenStates = {}

local function NextPopupAnimationToken(Frame)
    local Token = (PopupAnimationTokens[Frame] or 0) + 1
    PopupAnimationTokens[Frame] = Token
    return Token
end
...
function Library:ShowPopup(Frame)
    if not Frame then
        return
    end
    ...
    for _, Target in next, FadeTargets do
        ...
        if Target:IsA("GuiObject") then
            Properties.BackgroundTransparency = Target.BackgroundTransparency
        end
        if Target:IsA("ImageLabel") or Target:IsA("ImageButton") then
            Properties.ImageTransparency = Target.ImageTransparency
        end
        if Target:IsA("TextLabel") or Target:IsA("TextBox") or Target:IsA("TextButton") then
            Properties.TextTransparency = Target.TextTransparency
        end
        if Target:IsA("UIStroke") then
            Properties.Transparency = Target.Transparency
        end

        if next(Properties) then
            local OriginalProperties = PopupVisualState[Frame] and PopupVisualState[Frame][Target]
            if not OriginalProperties then
                OriginalProperties = {}
                PopupVisualState[Frame] = PopupVisualState[Frame] or {}
                PopupVisualState[Frame][Target] = OriginalProperties
            end

            for Property, Value in next, Properties do
                if OriginalProperties[Property] == nil then
    ...
    local DropPosition = UDim2.new(
        FinalPosition.X.Scale,
        FinalPosition.X.Offset,
        FinalPosition.Y.Scale,
        FinalPosition.Y.Offset - math.max(6, math.floor(10 * DPIScale))
    )
    local FrameVisualProperties = Targets[Frame]
    Targets[Frame] = nil

    Frame.Visible = true
    -- Keep the hitbox at its final position until the current input event has
    -- finished. Otherwise outside-click handlers can close the popup again
...
function Library:HidePopup(Frame)
    ...
    for Property, Value in next, FrameHideProperties do
        HideProperties[Property] = Value
    end

    local HideTween = Library:Tween(Frame, 0.12, HideProperties, Enum.EasingStyle.Linear, Enum.EasingDirection.In)
    if HideTween then
...
function Library:SetTextSize(Size)
    Size = math.clamp(math.floor((tonumber(Size) or Library.FontSize) + 0.5), 9, 24)
    ...
    for _, Descendant in next, ScreenGui:GetDescendants() do
        if Descendant:IsA("TextLabel") or Descendant:IsA("TextBox") or Descendant:IsA("TextButton") then
            pcall(function()
                Descendant.TextSize = ApplyTextScale(Size)
            end)
            Library:ApplyFontOffset(Descendant)
        end
    end
...
function Library:GetTextBounds(Text, Font, Size, Resolution)
    ...
    local CleanText = tostring(Text or ""):gsub("<%/?[%w:]+[^>]*>", "")
    local TextSize = ApplyTextScale(Library.FontSize)
    ...
    if typeof(Font) == "Font" then
        local success = pcall(function()
            ...
            Params.Width = math.max(1, MaxResolution.X)
    ...
    local FontValue = if typeof(Font) == "EnumItem" then Font else Enum.Font.Code
    Bounds = TextService:GetTextSize(CleanText, TextSize, FontValue, MaxResolution)
...
local GlowImageAsset = "rbxassetid://6906809185"

function Library:RegisterGlowTarget(Instance)
    if not Instance or not Instance:IsA("GuiObject") then
        return
    end

    if not table.find(Library.GlowTargets, Instance) then
        table.insert(Library.GlowTargets, Instance)
    end

    if Library.GlowEnabled then
        if not Library.GlowConnection then
            Library:SetGlowEnabled(true)
        else
            Library:UpdateGlow()
        end
    end
...
function Library:UpdateGlow()
    for Index = #Library.GlowTargets, 1, -1 do
        local Target = Library.GlowTargets[Index]
        if not Target or not Target.Parent then
            local GlowImage = Library.GlowImages[Target]
            if GlowImage then
                GlowImage:Destroy()
                Library.GlowImages[Target] = nil
            end
            table.remove(Library.GlowTargets, Index)
            continue
        end
        ...
        local GlowImage = Library.GlowImages[Target]
        if Library.GlowEnabled then
            if GlowImage and GlowImage.Parent ~= Parent then
                GlowImage:Destroy()
                GlowImage = nil
            end

            if not GlowImage then
                local LegacyGlow = Target:FindFirstChild("FormaGlow")
                if LegacyGlow then
                    LegacyGlow:Destroy()
                end

                GlowImage = Library:Create("ImageLabel", {
                    Name = "FormaGlow";
                    Active = false;
                    BackgroundTransparency = 1;
                    BorderSizePixel = 0;
                    Image = GlowImageAsset;
                    ScaleType = Enum.ScaleType.Stretch;
                    Parent = Parent;
                })
                Library.GlowImages[Target] = GlowImage
            end

            local Intensity = math.clamp(tonumber(Library.GlowIntensity) or 0, 0, 1)
            local IsFloatingPanel = Target == Library.Watermark or Target == Library.KeybindFrame
            local EffectiveIntensity = IsFloatingPanel and (Intensity * 0.25) or Intensity
            local Padding = math.floor((IsFloatingPanel and 3 or 8) + (EffectiveIntensity * (IsFloatingPanel and 8 or 16)))
            local ParentPosition = Vector2.zero
            if Parent:IsA("GuiObject") then
                ParentPosition = Parent.AbsolutePosition
            end

            GlowImage.ImageColor3 = Library.AccentColor
            GlowImage.ImageTransparency = IsFloatingPanel and (0.94 - (EffectiveIntensity * 0.35)) or (0.72 - (EffectiveIntensity * 0.5))
            GlowImage.Position = UDim2.fromOffset(
                Target.AbsolutePosition.X - ParentPosition.X - Padding,
                Target.AbsolutePosition.Y - ParentPosition.Y - Padding
            )
            GlowImage.Size = UDim2.fromOffset(
                Target.AbsoluteSize.X + (Padding * 2),
                Target.AbsoluteSize.Y + (Padding * 2)
            )
            GlowImage.ZIndex = math.max(0, Target.ZIndex - 1)
            GlowImage.Visible = Target.Visible
...
function Library:SetGlowEnabled(Enabled)
    Library.GlowEnabled = not not Enabled

    if Library.GlowEnabled and not Library.GlowConnection then
        Library.GlowConnection = RunService.RenderStepped:Connect(function()
            if Library.GlowEnabled then
                Library:UpdateGlow()
            end
        end)
    elseif not Library.GlowEnabled and Library.GlowConnection then
        Library.GlowConnection:Disconnect()
        Library.GlowConnection = nil
    end
...
function Library:SetGlowIntensity(Intensity)
    Library.GlowIntensity = math.clamp(tonumber(Intensity) or 0.35, 0, 1)
    Library:UpdateGlow()
end
...
function Library:Unload()
    if Library.GlowConnection then
        Library.GlowConnection:Disconnect()
        Library.GlowConnection = nil
    end
...
do
    local BaseAddonsFuncs = {}
        ...
        local ModePickerAnimationId = 0
        local ModePickerVisualState = {}
        local ModePickerOffset = math.max(6, math.floor(10 * DPIScale))

        local function GetModePickerPosition()
            return UDim2.fromOffset(ToggleLabel.AbsolutePosition.X + ToggleLabel.AbsoluteSize.X + 4, ToggleLabel.AbsolutePosition.Y)
        end

        local function GetModePickerVisualTargets()
            local Targets = { ModeSelectInner }
            for _, Descendant in next, ModeSelectInner:GetDescendants() do
                table.insert(Targets, Descendant)
            end
        ...
        local function PrepareModePickerVisuals()
            local Targets = {}
            ...
            for _, Target in next, GetModePickerVisualTargets() do
                ...
                if next(Properties) then
                    ...
                    if not OriginalProperties then
                        OriginalProperties = {}
                        ModePickerVisualState[Target] = OriginalProperties
                    end
        ...
        local function HideModePickerVisuals()
            for _, Target in next, GetModePickerVisualTargets() do
                local OriginalProperties = ModePickerVisualState[Target]
        ...
        local function AnimateModePicker(Visible)
            ModePickerAnimationId = ModePickerAnimationId + 1
            local AnimationId = ModePickerAnimationId
            local FinalPosition = GetModePickerPosition()
            ...
            if Visible then
                ...
                if ShowTween then
                    ShowTween.Completed:Connect(function(State)
                        if State == Enum.PlaybackState.Completed and ModePickerAnimationId == AnimationId then
        ...
        function KeyPicker:SetModePickerVisibility(bool)
            if bool then
                AnimateModePicker(true)
            else
                AnimateModePicker(false)
            end
        end
    ...
    function BaseAddonsFuncs:AddColorPicker(Idx, Info)
        ...
        do
            ...
            function ContextMenu:Show()
                ...
                Library:ShowPopup(self.Container)
            end

            function ContextMenu:Hide()
                if not self.Container.Visible and PopupOpenStates[self.Container] ~= "Opening" then
                    return
                end
                ...
                Library:HidePopup(self.Container)
            end
        ...
        function ColorPicker:Show()
            for Frame, Val in next, Library.OpenedFrames do
                if Frame.Name == "Color" then
                    Library:HidePopup(Frame)
                    Library.OpenedFrames[Frame] = nil
                end
            end

            Library:ShowPopup(PickerFrameOuter)
            Library.OpenedFrames[PickerFrameOuter] = true
        end

        function ColorPicker:Hide()
            Library:HidePopup(PickerFrameOuter)
        ...
        Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                ...
                if PickerFrameOuter.Visible
                    and PopupOpenStates[PickerFrameOuter] ~= "Opening"
                    and PopupOpenStates[PickerFrameOuter] ~= "Closing"
                    and (Mouse.X < AbsPos.X or Mouse.X > AbsPos.X + AbsSize.X
                    or Mouse.Y < (AbsPos.Y - 20 - 1) or Mouse.Y > AbsPos.Y + AbsSize.Y) then
                    ColorPicker:Hide()
                end

                if ContextMenu.Container.Visible
                    and PopupOpenStates[ContextMenu.Container] ~= "Opening"
                    and PopupOpenStates[ContextMenu.Container] ~= "Closing"
                    and not Library:MouseIsOverFrame(ContextMenu.Container) then
                    ContextMenu:Hide()
                end
            end
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton2
                ...
                and PopupOpenStates[ContextMenu.Container] ~= "Opening"
                and PopupOpenStates[ContextMenu.Container] ~= "Closing" then
    ...
    function BaseAddonsFuncs:AddDropdown(Idx, Info)
        ...
        local DropdownIndicator = Library:CreateLabel({
            AnchorPoint = Vector2.new(0, 0.5);
            BackgroundTransparency = 1;
        ...
        Library.RegistryMap[DropdownIndicator].Properties.TextColor3 = nil
        ...
        function Dropdown:UpdateColors()
            local TextColor = function()
                return Dropdown.Disabled and Library.DisabledTextColor or Color3.new(1, 1, 1)
            end

            ItemList.TextColor3 = TextColor()
            DropdownIndicator.TextColor3 = TextColor()
            Library.RegistryMap[ItemList].Properties.TextColor3 = TextColor
            Library.RegistryMap[DropdownIndicator].Properties.TextColor3 = TextColor
        end
        ...
        function Dropdown:Display()
            DropdownIndicator.Text = ListOuter.Visible and "-" or "+"
        ...
        InputService.InputBegan:Connect(function(Input)
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                ...
                if ListOuter.Visible
                    and PopupOpenStates[ListOuter] ~= "Opening"
                    and PopupOpenStates[ListOuter] ~= "Closing"
                    and (Mouse.X < AbsPos.X or Mouse.X > AbsPos.X + AbsSize.X
...
do
    ...
    function BaseGroupboxFuncs:AddButton(...)
        ...
        local function CreateBaseButton(Button)
            ...
            Library:AddToRegistry(Label, {
                TextColor3 = function()
                    return Button.Disabled and Library.DisabledTextColor or Color3.new(1, 1, 1)
                end;
            })
    ...
    function BaseGroupboxFuncs:AddToggle(Idx, Info)
        ...
        function Toggle:Display()
            if Toggle.Disabled then
                Library:Tween(ToggleLabel, 0.12, {
                    TextColor3 = Library.DisabledTextColor
                })
                Library:Tween(ToggleInner, 0.12, {
                    BackgroundColor3 = Toggle.Value and Library.DisabledAccentColor or Library.MainColor,
                    BorderColor3 = Library.DisabledOutlineColor
                })
            ...
            Library:Tween(ToggleLabel, 0.12, {
                TextColor3 = Toggle.Risky and Library.RiskColor or Color3.new(1, 1, 1)
            })
            Library:Tween(ToggleInner, 0.12, {
    ...
    function BaseGroupboxFuncs:AddSlider(Idx, Info)
        ...
        function Slider:UpdateColors()
            local TextColor = function()
                return Slider.Disabled and Library.DisabledTextColor or Color3.new(1, 1, 1)
            end

            if SliderText then
                SliderText.TextColor3 = TextColor()
                Library.RegistryMap[SliderText].Properties.TextColor3 = TextColor
            end
            DisplayLabel.TextColor3 = TextColor()
            Library.RegistryMap[DisplayLabel].Properties.TextColor3 = TextColor
        ...
        function Slider:Display(Instant)
            ...
            local FillSize = UDim2.new(X, 0, 1, 0)
            if Instant then
                Fill.Size = FillSize
            else
                Library:Tween(Fill, 0.12, {
                    Size = FillSize
                })
            end
    ...
    function BaseGroupboxFuncs:AddDropdown(Idx, Info)
        ...
        function Dropdown:UpdateColors()
            ...
            if DropdownLabel then
                ...
                Library.RegistryMap[DropdownLabel].Properties.TextColor3 = TextColor
            end
    ...
    function BaseGroupboxFuncs:AddDependencyGroupbox()
        ...
        function DepGroupbox:Resize()
            ...
            for _, Element in next, DepGroupbox.Container:GetChildren() do
                ...
                BoxOuter.Size = UDim2.new(1, -2 * DPIScale, 0, (10 * DPIScale + Size) + 2 + 2)
        end
...
do
    ...
    Library:RegisterGlowTarget(KeybindOuter)
...
do
    ...
    Library:RegisterGlowTarget(WatermarkOuter)
    ...
    Library.WatermarkIconImage = WatermarkIcon
    Library.WatermarkIconSource = CustomImageManagerAssets.WatermarkIcon.URL
    Library.WatermarkIconEnabled = true
    Library:MakeDraggable(Library.Watermark)

    local function UpdateWatermarkLayout()
        local IconOffset = WatermarkIcon.Visible and 48 or 0
        WatermarkLabel.Position = UDim2.new(0, 5 + IconOffset, 0, 0)
        WatermarkLabel.Size = UDim2.new(1, -(10 + IconOffset), 1, 0)
    end

    local LoadingWatermarkIcon = false

    function Library:SetWatermarkIcon(Source, Enabled)
        if typeof(Source) == "boolean" and Enabled == nil then
            Enabled = Source
            Source = nil
        end
        ...
        if Source == nil then
            Source = Library.WatermarkIconSource or CustomImageManagerAssets.WatermarkIcon.URL
        end
        if Enabled == nil then
            Enabled = true
        end

        local Image = WatermarkIcon.Image
        if Enabled and not LoadingWatermarkIcon then
            LoadingWatermarkIcon = true
            Image = nil
            if typeof(Source) == "string" and Source:match("^https?://") then
                CustomImageManager.SetAssetURL("WatermarkIcon", Source)
                Image = CustomImageManager.GetAsset("WatermarkIcon")
            elseif typeof(Source) == "string" and Source ~= "" then
                Image = Source
            end
            LoadingWatermarkIcon = false
        end

        Library.WatermarkIconSource = Source
        Library.WatermarkIconEnabled = not not Enabled
        WatermarkIcon.Image = Image or ""
        WatermarkIcon.Visible = Library.WatermarkIconEnabled and WatermarkIcon.Image ~= ""
        UpdateWatermarkLayout()

        if WatermarkLabel.Text ~= "" then
            Library:SetWatermark(WatermarkLabel.Text)
        end

        return WatermarkIcon.Visible
    end
    ...
    function Library:SetWatermark(Text)
        ...
        if Library.WatermarkIconEnabled and WatermarkIcon.Image == "" then
            Library:SetWatermarkIcon()
        end

        local X, Y = Library:GetTextBounds(Text, Library.Font, 14)
        local IconWidth = WatermarkIcon.Visible and 48 or 0
        Library.Watermark.Size = UDim2.new(0, math.max(200, X + IconWidth + 16), 0, math.max(44, (Y * 1.5) + 6))
...
do
    local ESPDefaults = {
        ...
        HealthLowColor = Color3.fromRGB(235, 70, 70);
        HealthHighColor = Color3.fromRGB(90, 220, 120);
        ChamsFillColor = Color3.fromRGB(255, 255, 255);
        ChamsFillTransparency = 0.55;
        ChamsOutlineColor = Color3.fromRGB(255, 255, 255);
        ChamsOutlineTransparency = 0.15;
    }

    local function CopyESPDefaults(Settings)
        Settings = typeof(Settings) == "table" and Settings or {}
        for Name, Value in next, ESPDefaults do
            if Settings[Name] == nil then
                Settings[Name] = Value
            end
        end
        if typeof(Settings.ElementPositions) ~= "table" then
            Settings.ElementPositions = {}
        end
        return Settings
    end

    local function CreateESPLineSet(Parent, Name, Color, ZIndex)
        local Lines = {}
    ...
    local function SetESPLineSetVisible(Lines, Visible)
        for _, Line in next, Lines do
            Line.Visible = Visible
        end
    end

    local function SetESPLineSet(Lines, Position, Size)
        local X = math.floor(Position.X + 0.5)
        local Y = math.floor(Position.Y + 0.5)
        local Width = math.max(math.floor(Size.X + 0.5), 1)
        local Height = math.max(math.floor(Size.Y + 0.5), 1)

        Lines.Top.Position = UDim2.fromOffset(X, Y)
        Lines.Top.Size = UDim2.fromOffset(Width, 1)
        Lines.Bottom.Position = UDim2.fromOffset(X, Y + Height - 1)
        Lines.Bottom.Size = UDim2.fromOffset(Width, 1)
        Lines.Left.Position = UDim2.fromOffset(X, Y)
        Lines.Left.Size = UDim2.fromOffset(1, Height)
        Lines.Right.Position = UDim2.fromOffset(X + Width - 1, Y)
        Lines.Right.Size = UDim2.fromOffset(1, Height)
    end

    local function GetESPBox(Character, Camera)
        local Success, BoxCFrame, BoxSize = pcall(Character.GetBoundingBox, Character)
        ...
        for _, X in ipairs({ -1, 1 }) do
            for _, Y in ipairs({ -1, 1 }) do
                for _, Z in ipairs({ -1, 1 }) do
                    local WorldPosition = (BoxCFrame * CFrame.new(HalfSize.X * X, HalfSize.Y * Y, HalfSize.Z * Z)).Position
                    local ViewportPosition = Camera:WorldToViewportPoint(WorldPosition)
                    if ViewportPosition.Z <= 0 then
                        return nil, nil
                    end

                    Minimum = Vector2.new(math.min(Minimum.X, ViewportPosition.X), math.min(Minimum.Y, ViewportPosition.Y))
                    Maximum = Vector2.new(math.max(Maximum.X, ViewportPosition.X), math.max(Maximum.Y, ViewportPosition.Y))
                end
            end
        end
    ...
    function Library:CreateESP(Info)
        Info = typeof(Info) == "table" and Info or {}
        local Settings = CopyESPDefaults(Info.Settings)
        for Name in next, ESPDefaults do
            if Info[Name] ~= nil then
                Settings[Name] = Info[Name]
            end
        end
        local ESP = {
            Name = tostring(Info.Name or "FormaESP");
            Settings = Settings;
            Entries = {};
            PreviewEntries = {};
            Labels = {};
        ...
        function ESP:GetGui()
            ...
            Overlay.Name = self.Name .. "Overlay"
        ...
        function ESP:CreateText(Name, ZIndex, Parent)
            local Label = Instance.new("TextLabel")
            Label.Name = Name
            Label.AnchorPoint = Vector2.new(0.5, 0.5)
            Label.BackgroundTransparency = 1
            Label.BorderSizePixel = 0
            Label.Size = UDim2.fromOffset(240, 15)
            ...
            Label.Visible = false
            Label.ZIndex = ZIndex
            self:ApplyFont(Label)
            Label.Parent = Parent or self:GetGui()
            return Label
        end
        ...
        function ESP:GetElementPosition(Name, Default)
            local Position = self.Settings.ElementPositions[Name] or Default
            Position = string.lower(tostring(Position or Default))
            if Position == "top" then return "Top" end
            if Position == "bottom" then return "Bottom" end
            if Position == "right" then return "Right" end
            return "Left"
        end

        function ESP:SetElementPosition(Name, Position)
            if typeof(self.Settings.ElementPositions) ~= "table" then
                self.Settings.ElementPositions = {}
            end
            Position = string.lower(tostring(Position or "Left"))
            if Position == "top" then
                self.Settings.ElementPositions[Name] = "Top"
            elseif Position == "bottom" then
                self.Settings.ElementPositions[Name] = "Bottom"
            elseif Position == "right" then
                self.Settings.ElementPositions[Name] = "Right"
            else
                self.Settings.ElementPositions[Name] = "Left"
            end
            return self.Settings.ElementPositions[Name]
        end

        function ESP:CreateCustomLabel(Entry, Name)
            local Definition = self.Labels[Name]
            if not Definition or Entry.Labels[Name] then
                return Entry.Labels[Name]
            end

            local Label = self:CreateText((Entry.Player and Entry.Player.Name or "ESP") .. Name, 6, Entry.Parent)
            Entry.Labels[Name] = Label
            return Label
        end

        function ESP:AddLabel(Name, Info)
            Name = tostring(Name or "")
            assert(Name ~= "" and Name ~= "Name" and Name ~= "Distance", "ESP label name must be unique")
            Info = typeof(Info) == "table" and Info or { Text = Info }
            local Definition = {
                Text = Info.Text or "";
                Color = Info.Color or Color3.new(1, 1, 1);
                Enabled = Info.Enabled;
                Position = Info.Position or "Bottom";
            }
            self.Labels[Name] = Definition
            if not table.find(self.LabelOrder, Name) then
                table.insert(self.LabelOrder, Name)
            end
            for _, Entry in next, self.Entries do
                self:CreateCustomLabel(Entry, Name)
            end
            for Entry in next, self.PreviewEntries do
                self:CreateCustomLabel(Entry, Name)
            end
            return Definition
        end

        function ESP:RemoveLabel(Name)
            Name = tostring(Name or "")
            self.Labels[Name] = nil
            for Index = #self.LabelOrder, 1, -1 do
                if self.LabelOrder[Index] == Name then
                    table.remove(self.LabelOrder, Index)
                end
            end
            for _, Entries in ipairs({ self.Entries, self.PreviewEntries }) do
                for Entry in next, Entries do
                    local Label = Entry.Labels[Name]
                    if Label then
                        Label:Destroy()
                        Entry.Labels[Name] = nil
                    end
                end
            end
        end

        function ESP:CreateEntry(Player, Parent, Preview)
            Parent = Parent or self:GetGui()
            local HealthOutline = Instance.new("Frame")
            HealthOutline.Name = Player.Name .. "HealthOutline"
            HealthOutline.BackgroundColor3 = Color3.new(0, 0, 0)
            HealthOutline.BorderSizePixel = 0
            HealthOutline.Visible = false
            HealthOutline.ZIndex = 4
            HealthOutline.Parent = Parent

            local HealthBack = Instance.new("Frame")
            HealthBack.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
            HealthBack.BorderSizePixel = 0
            HealthBack.ClipsDescendants = true
            HealthBack.Position = UDim2.fromOffset(1, 1)
            ...
            local Entry = {
                Player = Player;
                Parent = Parent;
                BoxOutline = CreateESPLineSet(Parent, Player.Name .. "BoxOutline", Color3.new(0, 0, 0), 3);
                Box = CreateESPLineSet(Parent, Player.Name .. "Box", Settings.BoxColor, 4);
                BoxInline = CreateESPLineSet(Parent, Player.Name .. "BoxInline", Color3.new(0, 0, 0), 5);
                Name = self:CreateText(Player.Name .. "Name", 5, Parent);
                Distance = self:CreateText(Player.Name .. "Distance", 5, Parent);
            ...
            for _, Name in ipairs(self.LabelOrder) do
                self:CreateCustomLabel(Entry, Name)
            end
            if Preview then
                self.PreviewEntries[Entry] = true
            else
        ...
        function ESP:DestroyCham(Entry)
            if Entry and Entry.Cham then
                pcall(function()
                    Entry.Cham:Destroy()
                end)
                Entry.Cham = nil
        ...
        function ESP:UpdateCham(Entry, Character, Visible)
            local Settings = self.Settings
            if not (Settings.Chams and Visible and Character) then
                self:DestroyCham(Entry)
                return
            end
            ...
            Cham.Adornee = Character
            Cham.DepthMode = Settings.ChamsAlwaysOnTop and Enum.HighlightDepthMode.AlwaysOnTop or Enum.HighlightDepthMode.Occluded
            Cham.FillColor = Settings.ChamsFillColor
            Cham.FillTransparency = math.clamp(tonumber(Settings.ChamsFillTransparency) or 0.55, 0, 1)
            Cham.OutlineColor = Settings.ChamsOutlineColor
            Cham.OutlineTransparency = math.clamp(tonumber(Settings.ChamsOutlineTransparency) or 0.15, 0, 1)
            Cham.Enabled = true
        end
        ...
        function ESP:HideEntry(Entry)
            ...
            SetESPLineSetVisible(Entry.BoxOutline, false)
            SetESPLineSetVisible(Entry.Box, false)
            SetESPLineSetVisible(Entry.BoxInline, false)
            Entry.Name.Visible = false
            Entry.Distance.Visible = false
            Entry.HealthOutline.Visible = false
            for _, Label in next, Entry.Labels do
                Label.Visible = false
            end
            self:DestroyCham(Entry)
        end
        ...
        function ESP:DestroyEntry(Entry)
            ...
            self:DestroyCham(Entry)
            for _, Lines in ipairs({ Entry.BoxOutline, Entry.Box, Entry.BoxInline }) do
                for _, Line in next, Lines do
                    pcall(function()
                        Line:Destroy()
                    end)
            ...
            for _, Object in ipairs({ Entry.Name, Entry.Distance, Entry.HealthOutline }) do
                pcall(function()
                    Object:Destroy()
                end)
            end
            ...
            self.PreviewEntries[Entry] = nil
            if Entry.Player and self.Entries[Entry.Player] == Entry then
                self.Entries[Entry.Player] = nil
            end
        end

        function ESP:RemovePlayer(Player)
            self:DestroyEntry(self.Entries[Player])
        end
        ...
        function ESP:SetOption(Name, Value)
            if ESPDefaults[Name] == nil then
                return false, "unknown ESP option " .. tostring(Name)
            end
        ...
        function ESP:SetOptions(Options)
            if typeof(Options) ~= "table" then
                return false, "expected an options table"
            end
        ...
        function ESP:SetEnabled(Visible)
            self.Settings.Enabled = not not Visible
        ...
        function ESP:SetFontSize(Size)
            self.Settings.TextSize = math.clamp(math.floor((tonumber(Size) or Library.FontSize) + 0.5), 8, 24)
            return self.Settings.TextSize
        end

        function ESP:ApplyEntryLayout(Entry, BoxPosition, BoxSize, Player, Character, Humanoid, Distance, HealthRatio)
            local Settings = self.Settings
            local BoxesVisible = Settings.Boxes == true
            SetESPLineSet(Entry.BoxOutline, BoxPosition - Vector2.one, BoxSize + Vector2.new(2, 2))
            SetESPLineSet(Entry.Box, BoxPosition, BoxSize)
            SetESPLineSet(Entry.BoxInline, BoxPosition + Vector2.one, BoxSize - Vector2.new(2, 2))
            SetESPLineSetVisible(Entry.BoxOutline, BoxesVisible)
            SetESPLineSetVisible(Entry.Box, BoxesVisible)
            SetESPLineSetVisible(Entry.BoxInline, BoxesVisible)
            for _, Line in next, Entry.Box do
                Line.BackgroundColor3 = Settings.BoxColor
            end

            local FontKey = tostring(Settings.Font or "ProggyClean") .. ":" .. tostring(Library.FontName)
            if Entry.FontName ~= FontKey then
                self:ApplyFont(Entry.Name)
                self:ApplyFont(Entry.Distance)
                for _, Label in next, Entry.Labels do
                    self:ApplyFont(Label)
                end
                Entry.FontName = FontKey
            end

            local TextSize = math.clamp(math.floor((tonumber(Settings.TextSize) or Library.FontSize) + 0.5), 8, 24)
            local LabelsByPosition = { Top = {}, Bottom = {}, Left = {}, Right = {} }
            local function AddLabel(Label, Position)
                if not Label then return end
                Label.TextSize = TextSize
                Label.Size = UDim2.fromOffset(220, TextSize + 4)
                table.insert(LabelsByPosition[Position] or LabelsByPosition.Left, Label)
            end
            ...
            Entry.Name.Text = tostring(Player.DisplayName or Player.Name)
            Entry.Name.TextColor3 = Settings.NameColor
            if Settings.Names == true then
                Entry.Name.Visible = true
                AddLabel(Entry.Name, self:GetElementPosition("Name", "Top"))
            end

            Entry.Distance.Text = string.format("%dm", math.floor(Distance + 0.5))
            Entry.Distance.TextColor3 = Settings.DistanceColor
            if Settings.Distance == true then
                Entry.Distance.Visible = true
                AddLabel(Entry.Distance, self:GetElementPosition("Distance", "Bottom"))
            end

            for _, Name in ipairs(self.LabelOrder) do
                local Definition = self.Labels[Name]
                local Label = Entry.Labels[Name]
                if Definition and Label then
                    local Enabled = Definition.Enabled
                    if typeof(Enabled) == "function" then
                        local Success, Value = pcall(Enabled, Player, Character, Humanoid, Distance)
                        Enabled = Success and Value or false
                    ...
                    if typeof(Text) == "function" then
                        local Success, Value = pcall(Text, Player, Character, Humanoid, Distance)
                        Text = Success and Value or ""
                    end
                    local Color = Definition.Color
                    if typeof(Color) == "function" then
                        local Success, Value = pcall(Color, Player, Character, Humanoid, Distance)
                        Color = Success and Value or Color3.new(1, 1, 1)
                    end

                    if Enabled and Text ~= nil and tostring(Text) ~= "" then
                        Label.Text = tostring(Text)
                        Label.TextColor3 = typeof(Color) == "Color3" and Color or Color3.new(1, 1, 1)
                        Label.Visible = true
                        AddLabel(Label, self:GetElementPosition(Name, Definition.Position))
                    end
                end
            end

            for Position, Labels in next, LabelsByPosition do
                for Index, Label in ipairs(Labels) do
                    local Offset = (Index - 1) * (TextSize + 2)
                    if Position == "Top" then
                        Label.AnchorPoint = Vector2.new(0.5, 1)
                        Label.Position = UDim2.fromOffset(BoxPosition.X + math.floor(BoxSize.X * 0.5), BoxPosition.Y - 3 - Offset)
                    elseif Position == "Bottom" then
                        Label.AnchorPoint = Vector2.new(0.5, 0)
                        Label.Position = UDim2.fromOffset(BoxPosition.X + math.floor(BoxSize.X * 0.5), BoxPosition.Y + BoxSize.Y + 3 + Offset)
                    elseif Position == "Right" then
                        Label.AnchorPoint = Vector2.new(0, 0.5)
                        Label.Position = UDim2.fromOffset(BoxPosition.X + BoxSize.X + 4, BoxPosition.Y + math.floor(BoxSize.Y * 0.5) + Offset)
                    else
                        Label.AnchorPoint = Vector2.new(1, 0.5)
                        Label.Position = UDim2.fromOffset(BoxPosition.X - 4, BoxPosition.Y + math.floor(BoxSize.Y * 0.5) + Offset)
                    end
                end
            end

            local HealthPosition = self:GetElementPosition("HealthBar", "Left")
            local Vertical = HealthPosition == "Left" or HealthPosition == "Right"
            if Vertical then
                Entry.HealthOutline.Position = UDim2.fromOffset(HealthPosition == "Left" and BoxPosition.X - 5 or BoxPosition.X + BoxSize.X + 1, BoxPosition.Y - 1)
                Entry.HealthOutline.Size = UDim2.fromOffset(4, BoxSize.Y + 2)
                Entry.HealthFill.AnchorPoint = Vector2.new(0, 1)
                Entry.HealthFill.Position = UDim2.fromScale(0, 1)
                Entry.HealthFill.Size = UDim2.new(1, 0, HealthRatio, 0)
                Entry.HealthGradient.Rotation = 90
            else
                Entry.HealthOutline.Position = UDim2.fromOffset(BoxPosition.X - 1, HealthPosition == "Top" and BoxPosition.Y - 5 or BoxPosition.Y + BoxSize.Y + 1)
                Entry.HealthOutline.Size = UDim2.fromOffset(BoxSize.X + 2, 4)
                Entry.HealthFill.AnchorPoint = Vector2.new(0, 0)
                Entry.HealthFill.Position = UDim2.fromScale(0, 0)
                Entry.HealthFill.Size = UDim2.new(HealthRatio, 0, 1, 0)
                Entry.HealthGradient.Rotation = 0
            end
            Entry.HealthFill.BackgroundColor3 = Color3.new(1, 1, 1)
            Entry.HealthGradient.Color = ColorSequence.new(Settings.HealthHighColor, Settings.HealthLowColor)
            Entry.HealthOutline.Visible = Settings.HealthBars == true and Humanoid ~= nil
        end

        function ESP:UpdateEntry(Player, Camera, CameraPosition, ViewportSize)
            local Entry = self.Entries[Player] or self:CreateEntry(Player)
            local Settings = self.Settings
            ...
            if not Character or not Root or not Root:IsA("BasePart") or Humanoid and Humanoid.Health <= 0 then
                self:HideEntry(Entry)
                return
            end

            if Settings.TeamCheck and LocalPlayer.Team ~= nil and Player.Team == LocalPlayer.Team then
                self:HideEntry(Entry)
                return
            end

            local Distance = (Root.Position - CameraPosition).Magnitude
            local MaxDistance = math.max(tonumber(Settings.MaxDistance) or ESPDefaults.MaxDistance, 0)
            if Distance > MaxDistance then
                self:HideEntry(Entry)
                return
            end

            local Position, Size = GetESPBox(Character, Camera)
            if not Position or not Size
                or Position.X + Size.X < 0 or Position.Y + Size.Y < 0
                or Position.X > ViewportSize.X or Position.Y > ViewportSize.Y then
                self:HideEntry(Entry)
                return
            end
            ...
            local BoxPosition = Vector2.new(math.floor(Position.X + 0.5), math.floor(Position.Y + 0.5))
            local BoxSize = Vector2.new(math.max(math.floor(Size.X + 0.5), 4), math.max(math.floor(Size.Y + 0.5), 8))
            self:ApplyEntryLayout(Entry, BoxPosition, BoxSize, Player, Character, Humanoid, Distance, HealthRatio)

            self:UpdateCham(Entry, Character, true)
        end
        ...
        function ESP:Update()
            ...
            for _, Player in ipairs(Players:GetPlayers()) do
                if Player ~= LocalPlayer then
                    self:UpdateEntry(Player, Camera, CameraPosition, ViewportSize)
                end
            end
        end
        ...
        function ESP:Destroy()
            ...
            for Player in next, self.Entries do
                self:RemovePlayer(Player)
            end
            ...
            if Library.ESP == self then
                Library.ESP = nil
            end
        end

        ESP.Connection = Players.PlayerRemoving:Connect(function(Player)
            ESP:RemovePlayer(Player)
        end)
        Library:GiveSignal(ESP.Connection)

        if Info.AutoUpdate ~= false then
            local UpdateConnection = RunService.RenderStepped:Connect(function()
                if not Library.Unloaded then
                    ESP:Update()
                end
            end)
            local PreviousDestroy = ESP.Destroy
            function ESP:Destroy()
        ...
        if Info.Primary ~= false then
            if Library.ESP and Library.ESP ~= ESP then
                Library.ESP:Destroy()
            end
            Library.ESP = ESP
        end
    ...
    function Library:BuildESPSection(Groupbox, ESP, Prefix)
        assert(Groupbox and typeof(Groupbox.AddToggle) == "function", "BuildESPSection expects a groupbox")
        ESP = ESP or Library.ESP
        assert(ESP and ESP.Settings, "BuildESPSection expects an ESP instance")

        Prefix = tostring(Prefix or "ESP")
        local Settings = ESP.Settings
        local Controls = {}
        local function Set(Name, Value)
            ESP:SetOption(Name, Value)
        end

        Controls.Enabled = Groupbox:AddToggle(Prefix .. "Enabled", {
            Text = "enabled";
        ...
        Controls.Boxes = Groupbox:AddToggle(Prefix .. "Boxes", {
            Text = "boxes";
            Default = Settings.Boxes;
            Callback = function(Value)
                Set("Boxes", Value)
            end;
        })
        Controls.Boxes:AddColorPicker(Prefix .. "BoxColor", {
            Default = Settings.BoxColor;
            Callback = function(Value)
                Set("BoxColor", Value)
            end;
        })
        Controls.Names = Groupbox:AddToggle(Prefix .. "Names", {
            Text = "names";
        ...
        Controls.Names:AddColorPicker(Prefix .. "NameColor", {
            Default = Settings.NameColor;
            Callback = function(Value)
                Set("NameColor", Value)
            end;
        })
        Controls.Distance = Groupbox:AddToggle(Prefix .. "Distance", {
            Text = "distance";
        ...
        Controls.Distance:AddColorPicker(Prefix .. "DistanceColor", {
            Default = Settings.DistanceColor;
            Callback = function(Value)
                Set("DistanceColor", Value)
            end;
        })
        Controls.Health = Groupbox:AddToggle(Prefix .. "Health", {
            Text = "health bars";
            Default = Settings.HealthBars;
            Callback = function(Value)
                Set("HealthBars", Value)
            end;
        })
        Controls.Health:AddColorPicker(Prefix .. "HealthLowColor", {
            Default = Settings.HealthLowColor;
            Callback = function(Value)
                Set("HealthLowColor", Value)
            end;
        })
        Controls.Health:AddColorPicker(Prefix .. "HealthHighColor", {
            Default = Settings.HealthHighColor;
            Callback = function(Value)
                Set("HealthHighColor", Value)
            end;
        })
        Controls.Chams = Groupbox:AddToggle(Prefix .. "Chams", {
            Text = "chams";
        ...
        Controls.Chams:AddColorPicker(Prefix .. "ChamsFill", {
            Default = Settings.ChamsFillColor;
            Transparency = Settings.ChamsFillTransparency;
            Callback = function(Value, Transparency)
                Set("ChamsFillColor", Value)
                Set("ChamsFillTransparency", Transparency or 0)
            end;
        })
        Controls.Chams:AddColorPicker(Prefix .. "ChamsOutline", {
            Default = Settings.ChamsOutlineColor;
            Transparency = Settings.ChamsOutlineTransparency;
            Callback = function(Value, Transparency)
                Set("ChamsOutlineColor", Value)
                Set("ChamsOutlineTransparency", Transparency or 0)
            end;
        })
        Controls.TeamCheck = Groupbox:AddToggle(Prefix .. "TeamCheck", {
            Text = "team check";
        ...
        Controls.Range = Groupbox:AddSlider(Prefix .. "Range", {
            Text = "max distance";
            Min = 100;
            Max = 10000;
        ...
        Controls.TextSize = Groupbox:AddSlider(Prefix .. "TextSize", {
            Text = "text size";
            Min = 8;
            Max = 24;
    ...
    -- This replaces the original mock preview with the same line, text and health-bar
    -- primitives used by the live ESP. The only 3D content is a clone of the local character.
    function Library:CreateESPViewer(Info)
        Info = typeof(Info) == "table" and Info or {}
        local ESP = Info.ESP or Library.ESP or Library:CreateESP({
            Name = "FormaESPViewer";
            Enabled = false;
            Primary = false;
        })
        ...
        local Outer = Library:Create("Frame", {
            ...
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderSizePixel = 0;
            Position = Info.Position or UDim2.new(1, -350, 0, 60);
            Size = Info.Size or UDim2.fromOffset(330, 420);
            Visible = Info.Visible ~= false;
            ZIndex = 350;
            Parent = ScreenGui;
        })
        Library:AddToRegistry(Outer, { BackgroundColor3 = "Black" }, true)
        Library:MakeDraggable(Outer, 28, false)
        ...
        Library:AddToRegistry(Inner, { BackgroundColor3 = "MainColor"; BorderColor3 = "AccentColor" }, true)
        ...
        Library:RemoveFromRegistry(Header)
        Library:AddToRegistry(Header, { TextColor3 = "FontColor" }, true)
        ...
        Library:AddToRegistry(PreviewOuter, { BackgroundColor3 = "Black" }, true)
        ...
        Library:AddToRegistry(Preview, { BackgroundColor3 = "BackgroundColor"; BorderColor3 = "OutlineColor" }, true)

        local WorldModel = Instance.new("WorldModel")
        WorldModel.Name = "LocalPlayerPreview"
        WorldModel.Parent = Preview
        local Camera = Instance.new("Camera")
        Camera.Parent = Preview
        Preview.CurrentCamera = Camera
        local PreviewEntry = ESP:CreateEntry(LocalPlayer, Preview, true)
        for _, Line in next, PreviewEntry.BoxOutline do Line.ZIndex = 355 end
        for _, Line in next, PreviewEntry.Box do Line.ZIndex = 356 end
        for _, Line in next, PreviewEntry.BoxInline do Line.ZIndex = 357 end
        PreviewEntry.Name.ZIndex = 358
        PreviewEntry.Distance.ZIndex = 358
        PreviewEntry.HealthOutline.ZIndex = 358
        PreviewEntry.HealthFill.Parent.ZIndex = 359
        PreviewEntry.HealthFill.ZIndex = 360

        local Footer = Library:CreateLabel({
            Position = UDim2.new(0, 8, 1, -37);
            Size = UDim2.new(1, -16, 0, 28);
            Text = "Drag a name, distance, health bar, or custom label to an edge";
            TextColor3 = Library.DisabledTextColor;
            TextXAlignment = Enum.TextXAlignment.Center;
            TextWrapped = true;
            ZIndex = 352;
        ...
        Library:RemoveFromRegistry(Footer)
        Library:AddToRegistry(Footer, { TextColor3 = "DisabledTextColor" }, true)

        local function DisconnectAll()
            for Index = #Viewer.Connections, 1, -1 do
                local Connection = table.remove(Viewer.Connections, Index)
                if Connection then Connection:Disconnect() end
            end
        end

        local function GetPreviewCharacter()
            return Viewer.PlayerModel
        end

        function Viewer:RefreshCharacter()
            if self.PlayerModel then
                self.PlayerModel:Destroy()
            ...
            Clone:PivotTo(CFrame.new(-Pivot.Position) * Pivot.Rotation)
            self.PlayerModel = Clone

            local _, Size = Clone:GetBoundingBox()
            local Height = math.max(Size.Y, 4)
            Camera.FieldOfView = 28
            Camera.CFrame = CFrame.lookAt(Vector3.new(Height * 1.15, Height * 0.62, Height * 2.5), Vector3.new(0, Height * 0.48, 0))
        end

        local function GetClosestEdge(InputPosition)
            local BoxPosition = PreviewEntry.Box.Top.AbsolutePosition
            local Width = PreviewEntry.Box.Top.AbsoluteSize.X
            local Height = PreviewEntry.Box.Left.AbsoluteSize.Y
            local Left = math.abs(InputPosition.X - BoxPosition.X)
            local Right = math.abs(InputPosition.X - (BoxPosition.X + Width))
            local Top = math.abs(InputPosition.Y - BoxPosition.Y)
            local Bottom = math.abs(InputPosition.Y - (BoxPosition.Y + Height))
            local Closest, Distance = "Left", Left
            if Right < Distance then Closest, Distance = "Right", Right end
            if Top < Distance then Closest, Distance = "Top", Top end
            if Bottom < Distance then Closest = "Bottom" end
            return Closest
        end

        function Viewer:BindElementDrag(Element, Name)
            if not Element or self.DragTargets[Element] then return end
            self.DragTargets[Element] = true
            Element.Active = true
            table.insert(self.Connections, Element.InputBegan:Connect(function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    self.Dragging = Name
                end
            end))
        ...
        table.insert(Viewer.Connections, InputService.InputChanged:Connect(function(Input)
            if Viewer.Dragging and (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) then
                ESP:SetElementPosition(Viewer.Dragging, GetClosestEdge(Input.Position))
                Viewer:Refresh()
            end
        end))
        table.insert(Viewer.Connections, InputService.InputEnded:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                Viewer.Dragging = nil
            end
        end))
        table.insert(Viewer.Connections, LocalPlayer.CharacterAdded:Connect(function()
            task.defer(function()
                if not Viewer.Destroyed then Viewer:RefreshCharacter() end
            end)
        end))

        function Viewer:Refresh()
            if self.Destroyed then return end
            if not GetPreviewCharacter() then
                self:RefreshCharacter()
            end
            local Character = GetPreviewCharacter()
            if not Character then
                ESP:HideEntry(PreviewEntry)
                return
            end

            local Size = Preview.AbsoluteSize
            if Size.X <= 0 or Size.Y <= 0 then return end
            local _, ModelSize = Character:GetBoundingBox()
            local Height = math.clamp(Size.Y * 0.62, 90, Size.Y - 56)
            local Width = math.clamp(Height * math.max(ModelSize.X, 1) / math.max(ModelSize.Y, 1) * 0.85, 46, Size.X - 76)
            local BoxSize = Vector2.new(math.floor(Width + 0.5), math.floor(Height + 0.5))
            local BoxPosition = Vector2.new(math.floor((Size.X - BoxSize.X) * 0.5), math.floor((Size.Y - BoxSize.Y) * 0.5))
            local Humanoid = Character:FindFirstChildOfClass("Humanoid")
            local HealthRatio = math.clamp(tonumber(Info.Health) or 72, 0, 100) / 100
            local Distance = tonumber(Info.Distance) or 128
            ESP:ApplyEntryLayout(PreviewEntry, BoxPosition, BoxSize, LocalPlayer, Character, Humanoid, Distance, HealthRatio)
            ESP:UpdateCham(PreviewEntry, Character, true)
            for _, Label in next, PreviewEntry.Labels do
                Label.ZIndex = 358
            end
            self:BindElementDrag(PreviewEntry.Name, "Name")
            self:BindElementDrag(PreviewEntry.Distance, "Distance")
            self:BindElementDrag(PreviewEntry.HealthOutline, "HealthBar")
            for Name, Label in next, PreviewEntry.Labels do
                self:BindElementDrag(Label, Name)
            end
        end

        function Viewer:SetVisible(Visible)
            Outer.Visible = not not Visible
        ...
        function Viewer:Destroy()
            ...
            self.Destroyed = true
            DisconnectAll()
            ESP:DestroyEntry(PreviewEntry)
            Outer:Destroy()
            Library.ESPViewers[self] = nil
        end

        Viewer.Frame = Outer
        Viewer.Preview = Preview
        Viewer.PlayerModel = nil
        Library.ESPViewers = Library.ESPViewers or {}
        Library.ESPViewers[Viewer] = true
        Viewer:RefreshCharacter()
        table.insert(Viewer.Connections, RunService.RenderStepped:Connect(function()
            Viewer:Refresh()
        end))
        Viewer:Refresh()
        return Viewer
    ...
    Library:OnUnload(function()
        ...
        for Viewer in next, Library.ESPViewers or {} do
            Viewer:Destroy()
        end
    end)
end
...
--// Target HUD \\--
do
    local TargetHUDOuter = Library:Create("Frame", {
        Name = "Indicator";
    ...
    Library:AddToRegistry(TargetHUDOuter, {
        BackgroundColor3 = "Black";
    }, true)

    local TargetHUDInner = Library:Create("Frame", {
        Name = "TargetHUD";
        BackgroundColor3 = Library.MainColor;
        BorderColor3 = Library.AccentColor;
        BorderMode = Enum.BorderMode.Inset;
        ...
        Size = UDim2.new(1, -2, 1, -2);
        ZIndex = 301;
        Parent = TargetHUDOuter;
    })

    Library:AddToRegistry(TargetHUDInner, {
        BackgroundColor3 = "MainColor";
        BorderColor3 = "AccentColor";
    }, true)

    local TargetHUDAccentLine = Library:Create("Frame", {
        BackgroundColor3 = Library.AccentColor;
        BorderSizePixel = 0;
        Position = UDim2.new(0, 0, 0, 0);
        Size = UDim2.new(1, 0, 0, 2);
        ZIndex = 302;
        Parent = TargetHUDInner;
    })

    Library:AddToRegistry(TargetHUDAccentLine, {
        BackgroundColor3 = "AccentColor";
    }, true)

    local TargetHUDAvatarOuter = Library:Create("Frame", {
        BackgroundColor3 = Color3.new(0, 0, 0);
        BorderSizePixel = 0;
    ...
    Library:AddToRegistry(TargetHUDAvatarOuter, { BackgroundColor3 = "Black" }, true)

    local TargetHUDAvatarInner = Library:Create("Frame", {
        BackgroundColor3 = Library.MainColor;
        BorderColor3 = Library.OutlineColor;
        BorderMode = Enum.BorderMode.Inset;
        Position = UDim2.fromOffset(1, 1);
        ...
        ClipsDescendants = true;
        ZIndex = 304;
        Parent = TargetHUDAvatarOuter;
    })
    Library:AddToRegistry(TargetHUDAvatarInner, {
        BackgroundColor3 = "MainColor";
        BorderColor3 = "OutlineColor";
    }, true)

    local TargetHUDAvatar = Library:Create("ImageLabel", {
        BackgroundTransparency = 1;
        Position = UDim2.fromOffset(2, 2);
        Size = UDim2.new(1, -4, 1, -4);
        ScaleType = Enum.ScaleType.Crop;
        ZIndex = 305;
        Parent = TargetHUDAvatarInner;
    })

    local TargetHUDDisplayName = Library:CreateLabel({
    ...
    local TargetHUDUsername = Library:CreateLabel({
        Position = UDim2.new(0, 65, 0, 23);
    ...
    local TargetHUDHealthOuter = Library:Create("Frame", {
        BackgroundColor3 = Color3.new(0, 0, 0);
        BorderColor3 = Color3.new(0, 0, 0);
        Position = UDim2.new(0, 65, 0, 47);
        Size = UDim2.new(1, -72, 0, 13);
        ZIndex = 303;
        Parent = TargetHUDInner;
    })

    Library:AddToRegistry(TargetHUDHealthOuter, {
        BorderColor3 = "Black";
    }, true)

    local TargetHUDHealthInner = Library:Create("Frame", {
        BackgroundColor3 = Library.MainColor;
        BorderColor3 = Library.OutlineColor;
        BorderMode = Enum.BorderMode.Inset;
        ClipsDescendants = true;
        Size = UDim2.new(1, 0, 1, 0);
        ZIndex = 304;
        Parent = TargetHUDHealthOuter;
    })

    Library:AddToRegistry(TargetHUDHealthInner, {
        BackgroundColor3 = "MainColor";
        BorderColor3 = "OutlineColor";
    }, true)

    local TargetHUDHealthFill = Library:Create("Frame", {
        BackgroundColor3 = Color3.new(1, 1, 1);
        BorderSizePixel = 0;
        Size = UDim2.new(0, 0, 1, 0);
        ZIndex = 305;
        Parent = TargetHUDHealthInner;
    })

    local TargetHUDHealthGradient = Library:Create("UIGradient", {
        Color = ColorSequence.new(Color3.fromRGB(90, 220, 120), Color3.fromRGB(235, 70, 70));
        Parent = TargetHUDHealthFill;
    })

    Library.TargetHUD = TargetHUDOuter
    Library.TargetHUDFrame = TargetHUDOuter
    Library.TargetHUDIndicator = TargetHUDOuter
    Library.TargetHUDContent = TargetHUDInner
    Library.TargetHUDTarget = nil
    Library.TargetHUDDisplayName = TargetHUDDisplayName
    Library.TargetHUDUsername = TargetHUDUsername
    Library.TargetHUDHealthFill = TargetHUDHealthFill
    Library.TargetHUDAvatar = TargetHUDAvatar
    Library.Indicators = Library.Indicators or {}
    Library.Indicators.TargetHUD = TargetHUDOuter

    local TargetHUDConnections = {}
    local TargetHUDHealthConnections = {}
    local CurrentHumanoid = nil
    local CurrentTarget = nil
    local TargetHUDAvatarRequest = 0
    ...
    local function ConnectTargetSignal(Signal, Callback, Connections)
    ...
    local function UpdateTargetHealth()
        ...
        if CurrentHumanoid and CurrentHumanoid.Parent then
            local MaxHealth = tonumber(CurrentHumanoid.MaxHealth) or 0
            local Health = tonumber(CurrentHumanoid.Health) or 0
            if MaxHealth > 0 then
                Ratio = math.clamp(Health / MaxHealth, 0, 1)
            end
        end

        Library:Tween(TargetHUDHealthFill, 0.14, {
            Size = UDim2.new(Ratio, 0, 1, 0);
        }, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
    end

    local function BindTargetCharacter(Character)
        DisconnectConnections(TargetHUDHealthConnections)
        CurrentHumanoid = nil
        ...
        if Character and Character:IsA("Model") then
            ...
            if Humanoid then
                ...
                ConnectTargetSignal(Humanoid:GetPropertyChangedSignal("Health"), UpdateTargetHealth, TargetHUDHealthConnections)
                ConnectTargetSignal(Humanoid:GetPropertyChangedSignal("MaxHealth"), UpdateTargetHealth, TargetHUDHealthConnections)
            else
                ConnectTargetSignal(Character.ChildAdded, function(Child)
                    if Child:IsA("Humanoid") then
                        BindTargetCharacter(Character)
                    end
                end, TargetHUDHealthConnections)
            end
        end
    ...
    local function ResolveTarget(Target)
        ...
        return Success and Player or nil, Character
    end

    local function UpdateTargetNames(Player, Character)
        local Username = Player and Player.Name or (Character and Character.Name) or "Unknown"
        local DisplayName = Player and Player.DisplayName or nil

        if not DisplayName and Character then
            DisplayName = Character:GetAttribute("DisplayName") or Character.Name
        end

        TargetHUDDisplayName.Text = tostring(DisplayName or Username)
        TargetHUDUsername.Text = "@" .. tostring(Username)
    end

    local function UpdateTargetAvatar(Player, Target)
        TargetHUDAvatarRequest = TargetHUDAvatarRequest + 1
        local Request = TargetHUDAvatarRequest
        TargetHUDAvatar.Image = ""
        ...
        task.spawn(function()
            ...
            if Success and Request == TargetHUDAvatarRequest and CurrentTarget == Target then
                TargetHUDAvatar.Image = Image
            end
        end)
    end

    function Library:SetTargetHUD(Target)
        if Target == nil or Target == false then
            return Library:HideTargetHUD()
        end

        local Player, Character = ResolveTarget(Target)
        if not Player and not Character then
            return false
        end

        DisconnectConnections(TargetHUDConnections)
        DisconnectConnections(TargetHUDHealthConnections)

        CurrentTarget = Target
        Library.TargetHUDTarget = Target
        UpdateTargetNames(Player, Character)
        UpdateTargetAvatar(Player, Target)
        BindTargetCharacter(Character)

        if Player then
            ConnectTargetSignal(Player:GetPropertyChangedSignal("DisplayName"), function()
                if CurrentTarget == Target then
                    UpdateTargetNames(Player, Character)
                end
            end, TargetHUDConnections)

            ConnectTargetSignal(Player.CharacterAdded, function(NewCharacter)
                if CurrentTarget == Target then
                    BindTargetCharacter(NewCharacter)
                end
            end, TargetHUDConnections)
        end

        TargetHUDOuter.Visible = true
        return true
    end

    function Library:HideTargetHUD()
        DisconnectConnections(TargetHUDConnections)
        DisconnectConnections(TargetHUDHealthConnections)
        ...
        TargetHUDAvatar.Image = ""
        Library.TargetHUDTarget = nil
        TargetHUDOuter.Visible = false
        TargetHUDHealthFill.Size = UDim2.new(0, 0, 1, 0)
        return true
    end

    function Library:SetTargetHUDVisibility(Visible)
        if Visible then
...
function Library:CreateWindow(...)
    ...
    Library:SetFont("ProggyClean")
    if Library.WatermarkIconEnabled ~= false and Library.WatermarkIconImage and Library.WatermarkIconImage.Image == "" then
        Library:SetWatermarkIcon()
    end
...
do
        ...
        Library:Create("UIPadding", {
            PaddingTop = UDim.new(0, DPIScale);
            PaddingBottom = UDim.new(0, DPIScale);
            PaddingLeft = UDim.new(0, DPIScale);
            PaddingRight = UDim.new(0, DPIScale);
            Parent = LeftSide;
        })
        ...
        function Tab:ShowTab()
            if Library.ActiveTabObject == Tab and TabFrame.Visible then
                return
            end

            local PreviousTab = Library.ActiveTabObject
            local Direction = 1
            if PreviousTab and Tab.Order < PreviousTab.Order then
                Direction = -1
            end

            if ActiveTabSwitchTween then
                ActiveTabSwitchTween:Cancel()
                ActiveTabSwitchTween = nil
            end

            for _, OtherTab in next, Window.Tabs do
                if OtherTab ~= Tab and OtherTab ~= PreviousTab then
                    OtherTab:HideTab()
                end
            end

            if PreviousTab and PreviousTab ~= Tab then
                PreviousTab:HideTab(true, Direction)
            end

            Library.ActiveTab = Tab.Name
        ...
        function Tab:HideTab(Animate, Direction)
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:AddTab(Name)
                ...
                local ButtonWidth = select(1, Library:GetTextBounds(Name, Library.Font, Library.FontSize))
                local Button = Library:Create("Frame", {
                    BackgroundColor3 = Library.MainColor;
                    BorderSizePixel = 0;
                    Size = UDim2.new(0, (ButtonWidth / DPIScale) + 8, 1, 0);
                    ZIndex = 6;
                    Parent = TabboxButtons;
                })
                ...
                local function ResizeTabboxButton()
                    Button.Size = UDim2.new(0, ButtonLabel.TextBounds.X + (8 * DPIScale), 1, 0)
                end

                ButtonLabel:GetPropertyChangedSignal("TextBounds"):Connect(ResizeTabboxButton)
                task.defer(ResizeTabboxButton)
                ...
                function Tab:Hide()
                    ...
                    ButtonLabel.TextColor3 = Library.DisabledTextColor
                    Library.RegistryMap[ButtonLabel].Properties.TextColor3 = "DisabledTextColor"
                end
